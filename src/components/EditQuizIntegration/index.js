...
const EditQuizIntegration = (props) => {
  const [courseId, setCourseId] = useState("");
  const [lessonId, setLessonId] = useState("");
  const [type, setType] = useState(props?.location?.state?.type);
  const [quizId, setQuizId] = useState(props?.location?.state?.quizId);
  ...
  const handleSubmit = () => {
    ...
    setIsSubmit(true);
    Api.patch(`api/v1/lessonQuiz/lesson/update`, {
      lessonQuizId: quizId,
      courseId: courseId,
  ...
  const questionList = () => {
    const userId = localStorage.getItem("userId");
    Api.get(`api/v1/lessonQuiz/${props?.location?.state?.quizId}`, {
      params: {
        courseLessonId: lessonId,
        userId: userId,
      },
        ...
        <div>
          <Row className="pb-2 d-flex justify-content-center align-items-center">
            <Col xs={12} sm={6}>
              ...
              <h6 style={{ color: "#1c1364" }}>Edit Questions For Quiz</h6>
            </Col>
            <Col xs={12} sm={6}>
...
export default EditQuizIntegration;
